...
const knownSchema = z.object({
  date: z
    ...
    .refine((v) => v.isAfter(dayjs(), 'day'), 'Ngày dự sinh phải ở tương lai')
    .refine(
...
export function DueDateUpdateDialog({
  ...
  const onSubmit = handleSubmit(async (values) => {
    ...
    useAuthStore.getState().updateProfile({ due_date: dueDate });
    close();
  });
    ...
    <Dialog open={open} onClose={close} fullWidth maxWidth="xs">
      <DialogContent>
        <Stack spacing={2} alignItems="center" sx={{ py: 1 }}>
          ...
          <CalendarMonthRounded sx={{ fontSize: 64, color: 'primary.light' }} />
            ...
            >
              ...
              {computedDueDate && (
                <Typography variant="body2" color="primary.main">
                  Dự sinh ước tính: {dayjs(computedDueDate).format('D/M/YYYY')}
                </Typography>
              )}
            </Stack>
          )}
